    ...
    public class ActionBinder
    {
        public ActionBindInfo ObterActionBindInfo(object controller, string path)
        {
            ...
            else
            {
                ...
                var tuplasNomeValor = ObterArgumetnonomeValores(queryString);
                var nomeArgumentos = tuplasNomeValor.Select(tupla => tupla.Nome).ToArray();
                var methosInfo = ObterMethodInfoAPartirDeNomeEArgumentos(nomeAction, nomeArgumentos, controller);
                return new ActionBindInfo(methosInfo, tuplasNomeValor);
            }
        }

        private IEnumerable<ArgumentoNomeValor> ObterArgumetnonomeValores(string queryString)
        {
        ...
        {
            var argumentosCount = argumentos.Count();
